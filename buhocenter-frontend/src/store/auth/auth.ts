...
const authModule: Module<CustomerStateInterface, any> = {
  ...
  mutations: {
    [AuthTypes.mutations.AUTH_GOOGLE_SUCCESS](
      state,
      data: CustomerStateInterface
    ) {
    ...
    [AuthTypes.mutations.SET_CUSTOMER_DATA](
      state,
  ...
  actions: {
    ...
    async [AuthTypes.actions.LOGIN_SOCIAL](
      { commit },
      social: string
    ): Promise<boolean> {
      try {
        const response:
          | ResponseAuth
          | boolean = await AuthRepository.loginWithSocial(social);
        if (response) {
