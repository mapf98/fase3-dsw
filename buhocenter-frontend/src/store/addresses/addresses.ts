...
const addresses: Module<AddressesStateInterface, any> = {
  ...
  actions: {
    async [AddressTypes.actions.CREATE_ADDRESS](
      { commit },
      address: Address
    ): Promise<boolean> {
      try {
        const response: Address[] = await addressRepository.createAddress(
          address
        );
        return true;
      } catch (e) {
    ...
    async [AddressTypes.actions.FETCH_ADDRESSES](
      { commit },
      customerId: number
    ): Promise<boolean> {
      try {
        const response: Address[] = await addressRepository.getCustomerAddresses(
          customerId
        );
    ...
    async [AddressTypes.actions.DELETE_ADDRESS](
      { commit },
      addressId: number
    ): Promise<boolean> {
      try {
    ...
    [AddressTypes.actions.SHOW_CREATE_ADDRESS_DIALOG](
      { commit },
      display: boolean
    ): void {
