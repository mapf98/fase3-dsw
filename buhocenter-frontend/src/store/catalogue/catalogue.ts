...
const catalogueModule: Module<CatalogueStateInterface, any> = {
  ...
  mutations: {
    [CatalogueTypes.mutations.SET_CATALOGUES](
      state,
      response: Catalogues
    ): void {
  ...
  actions: {
    async [CatalogueTypes.actions.FETCH_CATALOGUES](
      { commit },
      categoryId: string
    ): Promise<boolean> {
      try {
    ...
    async [CatalogueTypes.actions.FETCH_ALL_CATALOGUES]({
      commit,
    }): Promise<boolean> {
      try {
    ...
    async [CatalogueTypes.actions.SAVE_CATALOGUE](
      { commit },
