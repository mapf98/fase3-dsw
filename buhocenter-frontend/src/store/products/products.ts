...
const products: Module<ProductStateInterface, any> = {
  ...
  getters: {
    ...
    [ProductsTypes.getters.GET_PRODUCTS_DAILY_AND_PHOTOS_LOADED](
      state
    ): boolean {
  ...
  mutations: {
    ...
    [ProductsTypes.mutations.SET_PRODUCT_AND_PHOTOS_LOADED](
      state,
      loaded: boolean
    ): void {
    ...
    [ProductsTypes.mutations.SET_PRODUCTS_DAILY](
      state,
      products: Product[]
    ): void {
  ...
  actions: {
    async [ProductsTypes.actions.FETCH_PRODUCT_DETAIL](
      { commit },
      productId: number
    ): Promise<boolean> {
      try {
        const itemDetail: Product = await productsHttpRepository.getProductById(
          productId
        );
    ...
    //eliminar
    async [ProductsTypes.actions.FETCH_SERVICE_DETAIL](
      { commit },
      productId: number
    ): Promise<boolean> {
      try {
        const itemDetail = await servicesHttpRepository.getServiceById(
          productId
        );
        commit(ProductsTypes.mutations.SET_ITEM_DETAIL, itemDetail);
        return true;
    ...
    [ProductsTypes.actions.SET_PRODUCT_PHOTOS_NOT_LOADED](
      { commit },
      loaded: boolean
    ): void {
    ...
    async [ProductsTypes.actions.DELETE_PRODUCT](
      { commit },
      id
    ): Promise<boolean> {
      try {
    ...
    async [ProductsTypes.actions.FETCH_PRODUCTS_DAILY]({
      commit,
    }): Promise<boolean> {
      try {
    ...
    async [ProductsTypes.actions.UPLOAD_IMAGE](
      { commit },
      imageAndProduct
    ): Promise<boolean> {
      try {
        await productsFirebaseRepository.uploadImage(
    ...
    async [ProductsTypes.actions.SAVE_INVENTORY_QUANTITY](
      { commit },
      inventoryData
    ): Promise<boolean> {
      try {
    ...
    async [ProductsTypes.actions.UPDATE_INVENTORY_QUANTITY](
      { commit },
      inventoryData
    ): Promise<boolean> {
      try {
