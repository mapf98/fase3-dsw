...
import { LanguageResponse } from "@/store/languages/interfaces/LanguageResponse";
...
const languageModule: Module<LanguagesStateInterface, any> = {
  ...
  mutations: {
    ...
    [LanguageTypes.mutations.CHANGE_LANGUAGE_LOAD_SUCCESS](
      state,
      code: string
    ) {
  ...
  actions: {
    async [LanguageTypes.actions.API_GET_LANGUAGES]({
      commit,
    }): Promise<boolean> {
      try {
    ...
    async [LanguageTypes.actions.API_CHANGE_LANGUAGE](
      { commit, state },
      code: string
    ): Promise<boolean> {
      try {
