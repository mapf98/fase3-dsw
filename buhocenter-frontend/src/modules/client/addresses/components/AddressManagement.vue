<template>
  <v-container fluid>
          ...
          >
            <v-card-text class="font-weight-bold">
              ...
              <p
                ...
                class="text-center ma-0 caption text--primary"
              >
                <b>DEFAULT ADDRESS</b>
              </p>
            </v-card-text>
    ...
    <v-snackbar
      v-model="fetchingAddressesError"
      top
      :timeout="timeout"
      color="error"
    >
      {{ $t("ERROR_GET_ADDRESS") }}
      <v-btn color="white" text @click="fetchingAddressesError = false">{{
        $t("CLOSE")
      }}</v-btn>
    </v-snackbar>
    <v-snackbar
      v-model="deletingAddressError"
      top
      :timeout="timeout"
      color="error"
    >
      {{ $t("ERROR_DELETE_ADDRESS") }}
      <v-btn color="white" text @click="deletingAddressError = false">{{
        $t("CLOSE")
      }}</v-btn>
    </v-snackbar>
    <v-snackbar v-model="addressCreated" top :timeout="timeout" color="success">
      {{ $t("SUCCESS_ADDRESS") }}
      <v-btn color="white" text @click="addressCreated = false">{{
        $t("CLOSE")
      }}</v-btn>
    </v-snackbar>
    <v-snackbar
      v-model="addressCreatedError"
      top
      :timeout="timeout"
      color="error"
    >
      {{ $t("ERROR_PUT_ADDRESS") }}
      <v-btn color="white" text @click="addressCreatedError = false">{{
        $t("CLOSE")
      }}</v-btn>
    </v-snackbar>
  </v-container>
...
export default class AddressManagement extends Vue {
  ...
  async setDefaultAddress(addressId: number) {
    const updated: boolean = await this.SET_DEFAULT_ADDRESS(
      this.createDefaultAddressObject(addressId)
    );
  ...
  async saveChanges() {
    if (this.$refs.form.validate()) {
      const created: boolean = await this.CREATE_ADDRESS(
