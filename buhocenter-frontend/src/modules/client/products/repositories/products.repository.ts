...
class ProductsHttpRepository extends HttpRepository {
  ...
  public getProductById(id: number): Promise<Product> {
    return this.get(
      this.createUri([`${ProductsHttpRepository.RESOURCE}`, `${id}`])
    );
  }
  //tipar
  updateProductData(product: ProductCreate) {
    return this.patch(
      this.createUri([`${ProductsHttpRepository.RESOURCE}`], false),
      product,
      false
    );
  }
  ...
  public deleteProducts(id: number) {
    return this.delete(
      this.createUri([`${ProductsHttpRepository.RESOURCE}`, `${id}`], false),
      false
    );
  }

  public getProductsDailyRecommendation(): Promise<Product[]> {
    return this.get(
      this.createUri([
        `${ProductsHttpRepository.RESOURCE}`,
  ...
  public uploadImage(data: ProductPhotoDto) {
    return this.post(
      this.createUri([`${ProductsHttpRepository.RESOURCEIMAGE}`]),
      data
    );
  }
  ...
  public createDimension(data: dimensionDto) {
    return this.post(
      this.createUri([`${ProductsHttpRepository.RESOURCEDIMENSION}`]),
      data
    );
  }
  public saveInventary(data) {
    return this.post(
      this.createUri([`${ProductsHttpRepository.RESOURCEINVENTORY}`]),
      data
    );
  }
