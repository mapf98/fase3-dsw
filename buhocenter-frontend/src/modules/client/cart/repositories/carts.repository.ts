...
import { CartInterface } from "../interfaces/carts.interface";
...
class CartsHttpRepository extends HttpRepository {
   ...
   */
  public async addProductToCart(data: CART_INTERFACE.ProductCarts) {
    return await this.post(
      this.createUri([`${CartsHttpRepository.RESOURCE}`, "products"]),
      data,
      this.createHeader()
    );
  }
  ...
  public async addServiceToCart(data: CART_INTERFACE.ServiceCart) {
    return await this.post(
      this.createUri([`${CartsHttpRepository.RESOURCE}`, "services"]),
      data,
      this.createHeader()
    );
  }
   ...
   */
  public async getItemsCars(
    clientId: number
  ): Promise<CartInterface | boolean> {
    try {
      return await this.get(
        this.createUri([
          `${CartsHttpRepository.RESOURCE}`,
          `client/${clientId}`,
        ]),
        this.createHeader()
      );
