<template>
  <v-container fluid class="mt-5" style="max-width: none !important;">
      ...
      <v-col xs="12" sm="12" lg="6" md="6">
        <v-card fill-width class="pa-2">
          ...
          <v-form ref="form" v-model="isFormValid">
            ...
            <v-row
              v-if="!GET_CLIENT_DATA.is_federate"
            ...
            <v-row
              v-if="!this.GET_CLIENT_DATA.is_federate"
              class="mx-auto fill-width"
            >
              <v-col lg="6" xs="12">
                <v-text-field
              ...
              <v-col lg="6" xs="12">
                <v-text-field
                  ...
                  :append-icon="
                    showConfirmedPassword ? 'mdi-eye' : 'mdi-eye-off'
                  "
          ...
          <v-row class="d-flex justify-center my-2 mb-4" @click="saveChanges()">
            <v-btn :loading="loading" outlined color="primary">{{
              $t("SAVE")
            }}</v-btn>
          </v-row>
        </v-card>
    ...
    <v-snackbar
      v-model="userProfileModified"
      top
      :timeout="timeout"
      color="success"
    >
      Su perfil de usuario fue modificado exitosamente
      <v-btn color="white" text @click="userProfileModified = false"
        >Cerrar</v-btn
      >
    </v-snackbar>
    ...
    <v-snackbar
      v-model="loadingProfileError"
      top
      :timeout="timeout"
      color="error"
    >
      Ocurrió un error obteniendo la información del perfil del usuario
      <v-btn color="white" text @click="loadingProfileError = false"
        >Cerrar</v-btn
      >
    </v-snackbar>
  </v-container>
  ...
  async saveChanges() {
    if (this.$refs.form.validate()) {
      ...
      if (!updatedInFirebase) {
        ...
        const updated: boolean = await this.UPDATE_CUSTOMER(
          this.createClientDataObject()
        );
