...
export class CurrencyRepository extends HttpRepository
  ...
  private async _getExchange() {
    const data: { rates: { [key: string]: number } } = await this.get(
      CURRENCY_API
    );
  ...
  public change(
    ...
    if (this._exchange[toCurrency]) {
      amount =
        (amount * this._exchange[toCurrency]) / this._exchange[fromCurrency];
    }
  ...
  getCurrentSymbol(): string {
    return this._currentCurrency
      ? this._currentCurrency.symbol
      : AVAILABLE_CURRENCIES.USD.symbol;
  }
