...
describe('status service', () => {
   ...
   describe('create status history', () => {
       ...
       it('must try to save a status history', async () => {
           const statusHistory: StatusHistory = {payment: {id: 1}, status: {id: 1}} as StatusHistory;
       ...
       it('must return an error if it cannot be inserted', async () => {
           const statusHistory: StatusHistory = {payment: {id: 1}, status: {id: undefined}} as StatusHistory;
   ...
   describe('get status by checkout and status', () => {
        ...
        it('must return undefined if not found', async () => {
           ...
           expect(await service.getStatusHistoryByCheckoutIdAndStatusId(undefined, undefined)).toBeUndefined();
       });
    });
});
